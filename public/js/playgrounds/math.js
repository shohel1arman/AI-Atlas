...
(function () {
  ...
  window.GradientLab = function (canvas, onUpdate) {
    ...
    let lastFrame = 0, inView = true;
    const schedule = () => { if (running && inView && !document.hidden && !raf) raf = requestAnimationFrame(loop); };
    function loop(ts) {
      raf = 0;
      if (!running || !inView || document.hidden) return;
      if (ts - lastFrame >= 32) { lastFrame = ts; stepOnce(); draw(); }
      schedule();
    }
    ...
    return {
      draw,
      play() { if (!running) { running = true; lastFrame = 0; schedule(); } },
