...
(function () {
  ...
  window.NNPlayground = function (opts) {
    ...
    let raf = 0, lastFrame = 0, inView = true;
    const schedule = () => {
      if (state.running && inView && !document.hidden && !raf) raf = requestAnimationFrame(loop);
    };
    function loop(ts) {
      raf = 0;
      if (!state.running || !inView || document.hidden) return;
      if (ts - lastFrame < 32) { schedule(); return; }
      lastFrame = ts;
      state.loss = trainStep(state.net, state.data, state.lr); state.epoch++;
    ...
    return {
      ...
      play() { if (!state.running) { state.running = true; lastFrame = 0; schedule(); } },
      pause() { state.running = false; if (raf) cancelAnimationFrame(raf); raf = 0; },
