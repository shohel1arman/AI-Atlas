...
(function () {
  ...
  const intents = [
    { k: ['attention','transformer','self-attention','head'], title:'Transformers', href:'/modules/transformers/', say:'Attention lets every token look at every other token. Each emits a <b>query</b>, <b>key</b> and <b>value</b> — the dot-product of queries and keys is the attention weight. Opening the playground with the heatmap live →' },
    { k:['neural','backprop','network','perceptron','deep'], title:'Deep Learning', href:'/modules/deep-learning/', say:'A neural net stacks weighted sums + nonlinearities; <b>backpropagation</b> nudges weights down the loss gradient. Launching the Deep Learning Lab →' },
    { k:['regression','linear','fit','least squares'], title:'Machine Learning', href:'/modules/machine-learning/', say:'Linear regression finds the line minimizing squared error. Drag points and watch the fit chase your data → opening the ML Playground.' },
    { k:['vector','matrix','eigen','gradient','math','calculus'], title:'Mathematics', href:'/modules/mathematics/', say:'AI math is mostly linear algebra + calculus. Drag vectors, multiply matrices, roll a ball down a loss surface → opening Mathematics.' },
    { k:['diffusion','image','generate','gan','generative'], title:'Generative AI', href:'/modules/generative/', say:'Diffusion models reverse noise into an image step by step. Scrub the timeline → opening the Generative AI Lab.' },
    { k:['etl','pipeline','airflow','dag','data engineering'], title:'ETL', href:'/modules/etl/', say:'ETL = Extract, Transform, Load. Build a DAG and simulate data flow → opening ETL & Data Engineering.' },
    { k:['agent','llm','build llm','local','vram','gpu','fine-tune','lora','rlhf','quantize','quantization'], title:'LLM & Agents', href:'/modules/llm-agents/', say:'Build an LLM from tokenizer to RLHF, size it to your GPU\'s VRAM, and wire up a tool-using agent loop → opening the LLM & Agents Lab.' },
    { k:['shap','lime','explain','xai','saliency'], title:'Explainable AI', href:'/modules/xai/', say:'Explainability asks <i>why</i> a model decided. SHAP attributes each feature\'s push → opening Explainable AI.' },
    { k:['mlops','deploy','drift','monitor','registry'], title:'MLOps', href:'/modules/mlops/', say:'MLOps keeps models healthy in production: deploy, monitor, detect drift → opening MLOps.' },
    { k:['sql','data analysis','statistics','dashboard','query','distribution','a/b','ab test','visualize','chart'], title:'Data Analysis', href:'/modules/data-analysis/', say:'Run real SQL on a sample dataset and build a cross-filtering dashboard → opening Data Analysis.' },
    { k:['python','golang','go ','rust','code','compiler','numpy','pandas','pytorch','tensorflow','scikit','nosql','language','library'], title:'Code & Stack', href:'/modules/programming/', say:'Run Python, R and Go in a playground, explore the data-science stack (NumPy → PyTorch), and compare SQL vs NoSQL → opening Code & Stack.' },
  ];
