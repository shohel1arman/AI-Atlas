...
(function () {
  ...
  function watchVisibility(element, onChange) {
    let inView = true;
    const update = () => onChange(inView && !document.hidden && !reduceMotion);
  ...
  document.querySelectorAll('.hero-aurora, .scroll-cue, .marquee').forEach((element) => {
    watchVisibility(element, (active) => element.classList.toggle('motion-paused', !active));
  });
  ...
  if (canvas) {
    ...
    build();
    if (reduceMotion) {
      active = true; frame(0); active = false;
      if (raf) { cancelAnimationFrame(raf); raf = 0; }
    } else watchVisibility(canvas, (next) => {
      active = next;
  ...
  if (pcanvas) {
    ...
    let W, H, DPR, parts = [], raf = 0, active = false;
    function setup() {
      DPR = Math.min(window.devicePixelRatio || 1, 2);
    ...
    if (reduceMotion) {
      active = true; loop(); active = false;
      if (raf) { cancelAnimationFrame(raf); raf = 0; }
    } else watchVisibility(pcanvas, (next) => {
      active = next;
  ...
  if (mini) {
    ...
    let active = false;
    watchVisibility(mini, (next) => { active = next; });
    setInterval(() => {
      if (!active) return;
  ...
  if (spHeat) {
    let active = false;
    watchVisibility(spHeat, (next) => { active = next; });
    const cells = [];
    ...
    const paint = (force) => { if (active || force) cells.forEach(c => { const a = Math.random(); c.style.background = `rgba(${cols[(Math.random()*cols.length)|0]},${(0.12 + a * 0.85).toFixed(2)})`; }); };
    paint(true); setInterval(() => paint(false), 1300);
  }
  ...
  if (hero && window.matchMedia('(min-width: 981px)').matches) {
    ...
    let tx = 0, ty = 0, cxp = 0, cyp = 0, ticking = false;
    ...
    hero.addEventListener('pointerleave', () => { mxRaw = 0; myRaw = 0; requestTick(); });
    // magnetic buttons
    ...
    function tick() {
      ...
      const moving = Math.abs(mxRaw - tx) + Math.abs(myRaw - ty) + Math.abs(cursorX - cxp) + Math.abs(cursorY - cyp) > 0.12;
      if (moving && !document.hidden) requestAnimationFrame(tick);
      else ticking = false;
    }
  }
  ...
  if (fab) {
    ...
    const intents = [
      { k: ['attention', 'transformer', 'self-attention', 'head'], title: 'Self-Attention', href: '/modules/transformers/',
      ...
      { k: ['neural', 'backprop', 'network', 'perceptron', 'deep'], title: 'Neural Networks', href: '/modules/deep-learning/',
      ...
      { k: ['regression', 'linear', 'fit', 'least squares'], title: 'Linear Regression', href: '/modules/machine-learning/',
      ...
      { k: ['vector', 'matrix', 'eigen', 'gradient', 'math', 'calculus'], title: 'Mathematics', href: '/modules/mathematics/',
      ...
      { k: ['diffusion', 'image', 'generate', 'gan', 'generative'], title: 'Generative AI', href: '/modules/generative/',
      ...
      { k: ['etl', 'pipeline', 'data engineering', 'airflow', 'dag'], title: 'ETL', href: '/modules/etl/',
      ...
      { k: ['agent', 'llm', 'local', 'vram', 'gpu', 'lora', 'rlhf', 'quantize', 'build llm', 'fine-tune', 'fine tune'], title: 'LLM & Agents', href: '/modules/llm-agents/',
      ...
      { k: ['shap', 'lime', 'explain', 'xai', 'saliency'], title: 'Explainable AI', href: '/modules/xai/',
      ...
      { k: ['mlops', 'deploy', 'drift', 'monitor', 'registry'], title: 'MLOps', href: '/modules/mlops/',
    ...
    function respond(text) {
      ...
      setTimeout(() => {
        t.remove();
        if (hit) {
          add('bot', hit.say + `<div class="msg-cta"><a class="btn btn-primary btn-sm" href="${hit.href}">Open ${hit.title} ›</a></div>`);
        } else {
